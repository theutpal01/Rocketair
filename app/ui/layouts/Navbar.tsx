...
const Navbar = () => {
	...
	return (
		<div className={`${scrollDirection === "down" ? styles.active : styles.hidden} fixed z-50 w-full flex justify-between items-center h-20 duration-300 ease-in-out p-4 sm:p-6 md:px-8 lg:px-12 bg-transparent`}>
			...
			<div className='flex space-x-4 sm:space-x-6 md:space-x-8 lg:space-x-12'>
				...
				<div className='group flex justify-center items-center size-8 rotate-45 relative *:justify-between *:items-center *:flex *:absolute'>
					...
					<div className='w-full bg-inh *:bg-white *:mix-blend-difference *:size-2 *:rounded-full *:group-hover:bg-secondary *:duration-300 *:ease-in-out'>
						<div></div>
						<div></div>
					</div>
				</div>
